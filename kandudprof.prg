...
*select alltrim(name1)+' '+alltrim(name2)+' '+alltrim(name3);
       from(m.main) to file form\kandudprof.txt where potstan.and.(nest1='01' OR (nest1='00' AND nest3='ДЕК'))and;
       ( vstup='к' and vzvan='професор');          
       order by name1,name2,name3 noco 
*USE IN main
*=messagebox('Форма міститься в form\kandudprof.txt ')
ERASE form\kandudprof.doc
m.main='data\main'
m.nest3='dov\nest3'
m.kandudprof=TmpDir+"\kandudprof.dbf"
select alltrim(main.name1)+' '+alltrim(main.name2)+' '+alltrim(main.name3)+' - '+LEFT(nest3.povnest3,80);
       from(m.main),(m.nest3) INTO TABLE (m.kandudprof) ;
       where potstan.and.(main.nest1='01' OR (main.nest1='00' AND main.nest3='ДЕК'))and;
       ( main.vstup='к' and main.vzvan='професор') AND main.nest3=nest3.nest3 AND main.nest1=nest3.nest1;          
       order by main.nest3 noco 
USE IN main
USE IN nest3
SELECT ALLTRIM(STR(RECNO()))+". "+exp_1 FROM (m.kandudprof) TO FILE FORM\kandudprof.txt
COUNT TO m.TeachersCount
USE IN kandudprof
IF FILE('form\kandudprof.txt')
	...
	.Content.InsertAfter("Список професорів, кандидатів наук")
	...
	WITH oWord
	.Selection.EndKey(6)
	.Selection.InsertFile(FULLPATH('')+'form\kandudprof.txt')
	WITH .Selection.Find
	.ClearFormatting
  	.Text = [EXP_1]
    .Replacement.Text = []
	...
	.Execute(,,,,,,,,,,wdReplaceAll,,,,)
    ENDWITH
	.ActiveDocument.SaveAs(FULLPATH('')+'form\kandudprof.doc')
	.Quit
	ENDWITH

	ERASE form\kandudprof.txt

	=MESSAGEBOX('Список збережено у файлі form\kandudprof.doc')
ELSE
