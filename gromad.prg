...
*ENDIF
 ...
 m.gromad=m.shlyx+'\dov\gromad' 
 m.vstup=m.shlyx+'\dov\vstup'
USE (m.gromad) &Upd
=ApEBas()
WITH oMForm
	.Caption=[Громадянства - VKADR]
...
*	.AddObject('RegCB3','MyCB3') && Add Unsort command button
	...
	WITH .RegGrid.Column1
		...
		.FontSize=12
		.Width=150
		.ControlSource=[gromad.gromad]
		.Header1.FontSize=12
		.Header1.Caption=[Громадянство]
		.Header1.Alignment=2
	ENDWITH
...
FontSize=12
    PROC Click
		...
		SET INDEX TO (m.vstup) ORDER TAG nstup
*		REINDEX
		WITH Thisform
		.RegCB3.Enabled=.t.	
		.RegCB2.Enabled=.f.
