ERASE form\doktorprof.doc
m.main='data\main'
m.nest3='dov\nest3'
m.doctorprof=TmpDir+"\doctorprof.dbf"
select alltrim(main.name1)+' '+alltrim(main.name2)+' '+alltrim(main.name3)+' - '+LEFT(nest3.povnest3,80) ;
       from(m.main),(m.nest3) INTO TABLE (m.doctorprof) ;
       where potstan.and.(main.nest1='01' OR (main.nest1='00' AND main.nest3='ДЕК')) and;
       ( main.vstup='д' and main.vzvan='професор') AND main.nest3=nest3.nest3 AND main.nest1=nest3.nest1;          
       order by main.nest3 noco 
USE IN main
USE IN nest3
SELECT ALLTRIM(STR(RECNO()))+". "+exp_1 FROM (m.doctorprof) TO FILE FORM\doktorprof.txt
COUNT TO m.TeachersCount
USE IN doctorprof
IF FILE('form\doktorprof.txt')
	...
	.Content.InsertAfter("Список професорів, докторів наук")
	...
	WITH oWord
	.Selection.EndKey(6)
	.Selection.InsertFile(FULLPATH('')+'form\doktorprof.txt')
	WITH .Selection.Find
	.ClearFormatting
  	.Text = [EXP_1]
    .Replacement.Text = []
	...
	.Execute(,,,,,,,,,,wdReplaceAll,,,,)
    ENDWITH
	.ActiveDocument.SaveAs(FULLPATH('')+'form\doktorprof.doc')
	.Quit
	ENDWITH

	ERASE form\doktorprof.txt
	ERASE (m.doctorprof)

	=MESSAGEBOX('Список збережено у файлі form\doktorprof.doc')
ELSE
