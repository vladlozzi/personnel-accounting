...
*select alltrim(name1)+' '+alltrim(name2)+' '+alltrim(name3);
       from(m.main) to file form\docentbez.txt where potstan.and.(nest1='01' OR (nest1='00' AND nest3='ДЕК'))and;
       ( EMPTY(vstup) and vzvan='доцент');          
       order by name1,name2,name3 noco 
*USE IN main
*=messagebox('Форма міститься в form\docentbez.txt ')
...
select alltrim(main.name1)+' '+alltrim(main.name2)+' '+alltrim(main.name3)+' - '+LEFT(nest3.povnest3,80);
       ...
       ( EMPTY(main.vstup) and main.vzvan='доцент') AND main.nest3=nest3.nest3 AND main.nest1=nest3.nest1;          
...
IF FILE('form\docentbez.txt')
	...
	.Content.InsertAfter("Список доцентів без наукового ступеня")
	...
	=MESSAGEBOX('Список збережено у файлі form\docentbez.doc')
ELSE
