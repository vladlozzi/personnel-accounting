...
a.stod,a.oklad,000.00 as pdopl,000.00 as pnadb,000.00 as nadbdop,e.posada,e.rang from (m.main) a, (m.posad) e ;
where a.nest1==e.nest1 and a.pos=e.pos AND potstan ;
...
*			m.okl1=m.dopl+m.nadb
			...
			oWordDoc1.Content.InsertAfter(' '+a.sprava) && 2
...
*			oWordDoc1.Content.InsertAfter(SPACE(1)+STR(a.pdopl,6,2)) && PICT '99.9') 72
*			oWordDoc1.Content.InsertAfter(SPACE(1)+STR(a.pnadb,6,2)) && PICT '99.9') 79
...
oWordDoc1.Content.InsertParagraphAfter
oWordDoc1.Content.InsertAfter([|      Номер       |       Прізвище,ім'я,         |                    |]) &&  К-ть  |Посадо-|Допл. і надб. |Місячний  |У т.ч.  |     |])
oWordDoc1.Content.InsertParagraphAfter
oWordDoc1.Content.InsertAfter('|     особової     |        по-батькові           |       Посада       |') && посадо-|вий    |до окл.(грн.) |фонд заро-|доплата,|При- |')
oWordDoc1.Content.InsertParagraphAfter
oWordDoc1.Content.InsertAfter('|      справи      |                              |                    |') && вих    |оклад, |--------------|бітної    |надбав- |мітка|')
oWordDoc1.Content.InsertParagraphAfter
