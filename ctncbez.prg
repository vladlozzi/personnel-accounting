ERASE form\ctncbez.doc
m.main='data\main'
m.nest3='dov\nest3'
m.ctncbez=TmpDir+"\ctncbez.dbf"
select alltrim(main.name1)+' '+alltrim(main.name2)+' '+alltrim(main.name3)+' - '+LEFT(nest3.povnest3,80);
       from(m.main),(m.nest3) INTO TABLE (m.ctncbez) ;
       where potstan.and.(main.nest1='01' OR (main.nest1='00' AND main.nest3='ДЕК'))and;
       ( EMPTY(main.vstup) and main.vzvan='ст.н.с.') AND main.nest3=nest3.nest3 AND main.nest1=nest3.nest1;          
       order by main.nest3 noco 
USE IN main
USE IN nest3
SELECT ALLTRIM(STR(RECNO()))+". "+exp_1 FROM (m.ctncbez) TO FILE FORM\ctncbez.txt
COUNT TO m.TeachersCount
USE IN ctncbez
IF FILE('form\ctncbez.txt')
	...
	.Content.InsertAfter("Список старших наукових співробітників без наукового ступеня ")
	...
	WITH oWord
	.Selection.EndKey(6)
	.Selection.InsertFile(FULLPATH('')+'form\ctncbez.txt')
	WITH .Selection.Find
	.ClearFormatting
  	.Text = [EXP_1]
    .Replacement.Text = []
	...
	.Execute(,,,,,,,,,,wdReplaceAll,,,,)
    ENDWITH
	.ActiveDocument.SaveAs(FULLPATH('')+'form\ctncbez.doc')
	.Quit
	ENDWITH
	ERASE form\ctncbez.txt

	=MESSAGEBOX('Список збережено у файлі form\ctncbez.doc')
