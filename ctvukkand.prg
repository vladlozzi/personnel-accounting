...
*select alltrim(name1)+' '+alltrim(name2)+' '+alltrim(name3);
       from(m.main) to file form\ctvukkand.txt where potstan.and.(nest1='01' OR (nest1='00' AND nest3='ДЕК'))and;
       (LEFT(vstup,1)='к' and pos=12);          
       order by name1,name2,name3 noco 
*USE IN main
*=messagebox('Форма міститься в form\ctvukkand.txt ')
ERASE form\ctvukkand.doc
m.main='data\main'
m.nest3='dov\nest3'
m.ctvukkand=TmpDir+"\ctvukkand.dbf"
select alltrim(main.name1)+' '+alltrim(main.name2)+' '+alltrim(main.name3)+' - '+LEFT(nest3.povnest3,80);
       from(m.main),(m.nest3) INTO TABLE (m.ctvukkand) ;
       where potstan.and.(main.nest1='01' OR (main.nest1='00' AND main.nest3='ДЕК'))and;
       ( LEFT(main.vstup,1)='к' and main.pos=12) AND main.nest3=nest3.nest3 AND main.nest1=nest3.nest1;          
       order by main.nest3 noco 
USE IN main
USE IN nest3
SELECT ALLTRIM(STR(RECNO()))+". "+exp_1 FROM (m.ctvukkand) TO FILE FORM\ctvukkand.txt
COUNT TO m.TeachersCount
USE IN ctvukkand
IF FILE('form\ctvukkand.txt')
	...
	.Content.InsertAfter("Список старших викладачів , кандидатів наук")
	...
	WITH oWord
	.Selection.EndKey(6)
	.Selection.InsertFile(FULLPATH('')+'form\ctvukkand.txt')
	WITH .Selection.Find
	.ClearFormatting
  	.Text = [EXP_1]
    .Replacement.Text = []
	...
	.Execute(,,,,,,,,,,wdReplaceAll,,,,)
    ENDWITH
	.ActiveDocument.SaveAs(FULLPATH('')+'form\ctvukkand.doc')
	.Quit
	ENDWITH

	ERASE form\ctvukkand.txt
	ERASE (m.ctvukkand)

	=MESSAGEBOX('Список збережено у файлі form\ctvukkand.doc')
ELSE
